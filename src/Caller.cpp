...
bool Caller::parseOptions(int argc, char** argv) {
	...
	*/
	...
	if (cmd.optionSet("fido-protein") || cmd.optionSet("picked-protein")) {
		...
		std::string protEstimatorDecoyPrefix = "random_"; // 反库蛋白标识
		...
		if (cmd.optionSet("fido-protein")) {
			...
			// Use the picked protein-level FDR to infer protein probabilities. Provide the fasta file as the argument
			// to this flag, which will be used for protein grouping based on an in-silico digest. If no fasta file is
			// available or protein grouping is not desired, set this flag to \"auto\" to skip protein grouping.
			...
			// then protein A is eliminated and all the peptides are considered as evidence for protein B. Note that this filtering
			// is done based on the complete set of peptides in the database, not based on the identified peptides in the search results.
			// Alternatively, if this option is set and if all of the identified peptides associated with protein B are also associated
			// with protein A, then Percolator will report a comma-separated list of protein IDs, where the full-length protein B is
			// first in the list and the fragment protein A is listed second. Commas inside protein IDs will be replaced by semicolons. Not available for Fido.
			bool pickedProteinReportFragmentProteins = false;
			// If this option is set and multiple database proteins contain exactly the same set of peptides, then the IDs of
			// these duplicated proteins will be reported as a comma-separated list, instead of the default behavior of randomly
			// discarding all but one of the proteins. Commas inside protein IDs will be replaced by semicolons. Not available for Fido.
